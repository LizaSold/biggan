...
def prepare_parser():
  ...
  parser.add_argument(
    '--dataset', type=str, default='ecoset',
...
def run(config):
  ...
  for i,(x,y) in enumerate(tqdm(train_loader)):
    ...
    if i==0:
      with h5.File(config['data_root'] + '/ILSVRCECO.hdf5' % config['image_size'], 'w') as f:
    ...
    # Else append to the hdf5
      ...
      with h5.File(config['data_root'] + '/ILSVRCECO.hdf5' % config['image_size'], 'a') as f:
